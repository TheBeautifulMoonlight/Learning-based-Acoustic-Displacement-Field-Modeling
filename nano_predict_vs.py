...
train_x = [[data_x1[i],data_x2[i]] for i in range(len(data))][:90]
train_y = [[data_y[i]] for i in range(len(data))][:90]
train_x = torch.tensor(train_x).float()
train_y = torch.tensor(train_y).float()
test_x = [[data_x1[i],data_x2[i]] for i in range(len(data))][90:]
test_y = [[data_y[i]] for i in range(len(data))][90:]
...
test_x = [[data_x1[i],data_x2[i]] for i in range(len(data))]
test_x = torch.tensor(test_x).float()
...
fig = plt.figure()
ax = Axes3D(fig)
ax.scatter(data_x1, data_x2, data_y,c='blue')
ax.scatter(data_x1, data_x2, prediction,c='red')
ax.set_zlabel(predict_name, fontdict={'size': 15, 'color': 'red'})
ax.set_ylabel('Y', fontdict={'size': 15, 'color': 'red'})
ax.set_xlabel('X', fontdict={'size': 15, 'color': 'red'})
'''保存预测数据'''
csv_file = open('PredictCompare'+predict_name+'2.csv', 'w', newline='', encoding='gbk')
writer = csv.writer(csv_file)
draw_x1 = list(data_x1[90:])
draw_x2 = list(data_x2[90:])
draw_x1.insert(0,'x')
